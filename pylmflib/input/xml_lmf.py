...
from pylmflib.utils.xml_format import parse_xml
from pylmflib.utils.error_handling import InputError
...
def factory(object_name, attributes):
    ...
    running_path = os.path.join(file_path, "..")
    if os.name == 'posix':
        # Unix-style path
        separator = '/'
    else:
        # Windows-style path
        separator = '\\'
    pattern = running_path + separator + ".." + separator + "pylmflib" + separator + "core" + separator + module_name + ".py"
    full_path = glob.glob(pattern)
    if len(full_path) < 1:
        # No file with this name exists
        raise InputError(pattern + ".py", "No file named '%s' exists in the library. It is not allowed, so please solve this issue by renaming files correctly." % (module_name + ".py"))
...
def get_sub_elements(instance, element):
    ...
    for sub_element in element:
        ...
        if sub_element.tag == "feat":
            ...
            instance.set_feat(sub_element.attrib["att"], sub_element.attrib["val"], True)
        else:
            ...
            if type(attr_value) is list:
                ...
                # Normaly only Lexicon for lexicalEntries
                lexeme = None
                partOfSpeech = None
                for lemma in sub_element.findall("Lemma"):
                    for feats in lemma.findall("feat"):
                        att = feats.attrib
                        # print("feat: ", att)
                        if(feats.attrib['att'] == 'lexeme'):
                            lexeme = feats.attrib['val']
                        if(feats.attrib['att'] == 'partOfSpeech'):
                            partOfSpeech = feats.attrib['val']

                key = (lexeme, partOfSpeech)
                attr_value[key] = sub_instance
            else:
